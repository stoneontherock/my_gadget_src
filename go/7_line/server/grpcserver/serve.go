...
func Serve() {
	lis, err := net.Listen("tcp", model.GRPCListenAddr) //1.指定监听地址:端口号
	panicerr.Handle(err, "grpcserver:Serve:net.Listen")

	s := grpc.NewServer()                      //2.新建gRPC实例
	pb.RegisterChannelServer(s, &grpcServer{}) //3.在gRPC服务器注册我们的服务实现。参数2是接口(满足服务定义的方法)。在.pb.go文件中搜索Register关键字即可找到这个函数签名
	err = s.Serve(lis)
